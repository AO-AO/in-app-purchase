...
function gen_error(res) {
	let res_err = res.body && res.body.error;
...
module.exports.refreshToken = function (configIn, cb) {

	if (!checkSubscriptionState && !configIn) {
		return cb(new Error('missing google play api info'), {
			status: constants.VALIDATION.FAILURE,
			message: 'client_id, client_secret, access_token and refres_token should be provided'
		});
	...
	refreshGoogleTokens(configIn, function (error, res, body) {
		if (error) {
...
function refreshGoogleTokens(configIn, cb) {
	...
	if (!configIn) {
		...
		body[KEYS.GRANT_TYPE] = KEYS.REFRESH_TOKEN;
		body[KEYS.CLIENT_ID] = configIn.googleClientID;
		body[KEYS.CLIENT_SECRET] = configIn.googleClientSecret;
		body[KEYS.REFRESH_TOKEN] = configIn.googleRefToken;
	}
